...
public class SecurityConfig  {
    ...
    protected SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        //SESSION 비활성화 //1
        http.sessionManagement((sesssionConfig)->{
            sesssionConfig.sessionCreationPolicy(SessionCreationPolicy.STATELESS); //세션 상태 x
        });

        //TokenFilter 추가
        http.addFilterBefore(jwtAuthorizationFilter,  LogoutFilter.class); //로그아웃 이전 필터 작동
